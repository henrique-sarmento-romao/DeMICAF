...
def compute_feature_histograms(
    ...
    if len(bin_edges) != d:
        raise ValueError(
            f"Expected {d} sets of bin edges, received {len(bin_edges)}."
        )
    ...
    for j in range(d):
        ...
        if not np.all(np.diff(edges) > 0):
            raise ValueError(
                f"Bin edges for feature {j} must be strictly increasing."
            )
...
def dp_feature_histograms(
    ...
    for client in client_histograms:
        if len(client) != d:
            raise ValueError(
                "Every client must provide one histogram per feature."
            )
    ...
    for client in client_histograms:
        for j in range(d):
            if len(client[j]) != bin_counts[j]:
                raise ValueError(
                    f"Inconsistent histogram size for feature {j}."
                )

    global_histograms = [
        np.sum(
            [
                np.asarray(client[j], dtype=float)
                for client in client_histograms
            ],
            axis=0,
        )
    ...
    l2_sensitivity = (
        np.sqrt(2.0 * d)
        if replacement_adjacency
        else np.sqrt(float(d))
    )
    ...
    noisy_histograms = [
        np.maximum(histogram, 0.0)
        for histogram in noisy_histograms
    ]
